...
mod tests {
    ...
    async fn test_attributes_match() {
        if !std::env::var("L1_TEST_RPC_URL").is_ok() {
            return;
        }
        ...
        let rpc_url = reqwest::Url::parse(&env::var("L2_TEST_RPC_URL").unwrap()).unwrap();
    ...
    async fn get_expected_hashes(block_num: u64) -> Vec<B256> {
        let rpc_url = reqwest::Url::parse(&env::var("L2_TEST_RPC_URL").unwrap()).unwrap();
        let provider: ReqwestProvider = ProviderBuilder::default().on_http(rpc_url);
        let mut txs = provider
            .get_block(block_num.into(), true)
            .await
            .unwrap()
            .unwrap()
            .transactions;
        txs.convert_to_hashes();
        let BlockTransactions::Hashes(hashes) = txs else {
            panic!("failed to get expected hashes")
        };
        hashes
    }

    fn get_tx_hashes(txs: &[RawTransaction]) -> Vec<B256> {
        txs.iter().map(|tx| keccak256(&tx.0)).collect()
    }
}
